...
#define MAX_PROD 51 // Tamanho maximo da array: int cont e contcar

// Variaveis globais
int cont = 0; // para armazenar a quantidade de produtos cadastrados.
int contcar = 0; //para armazenar a quantidade de produtos dentro do carrinho.
...
void listarProduto (Produto prod[MAX_PROD], int *cont) {
    if (*cont <= 0) { // Verifica se a lista de produtos está vazia.
        printf("\nNenhum produto cadastrado.\n");
    } else {
        printf("\n-----------Lista de Produtos-----------\n");
        for (int i = 0; i < *cont; i++) { // Exibi todos os produtos cadastrados
            printf("Código: %d\n", prod[i].codigo);
            printf("Nome: %s\n", prod[i].nome);
            printf("Preço: R%.2f\n", prod[i].preco);
            printf("--------------------------------------\n");
...
void visualizarListaProduto (Produto prod[MAX_PROD], int *cont) {
    bool sair; // Variável para controlar a saída do loop
...
// Funções para ver o carrinho ou perguntar se quer ver o carrinho
void listarCarrinho (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    if (*contcar <= 0) { // Verifica se o carrinho está vazio
        printf("\nNenhum produto foi inserido no Carrinho.\n");
    } else {
        printf("\n-------------------Carrinho-------------------\n");
        for (int i = 0; i < *contcar; i++) { // Exibi todos os produtos no carrinho
...
void visualizarCarrinho (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    bool sair; // Variável para controlar a saída do loop
    char escolha[3]; // Array para armazenar a escolha do usuário
    do {
        ...
        switch (escolha[0]) {
        case '1':
            listarCarrinho(prod, cont, car, contcar);
            sair = true;
            break;
...
// Funções sobre o Produto, acesso restrito, apenas admin pode alterar
void cadastrarProduto (Produto prod[MAX_PROD], int *cont) {
    bool sair = false; 
    if (*cont > 49) { // Verifica se atingiu o limite de produtos cadastrados
    ...
    do {
        ...
        prod[*cont].codigo = atoi(validar); // Converte a string validar para inteiro e adicionar o valor na variavel prod[*cont].codigo  

        sair = true;
        for (int i = 0; i < *cont; i++) {
            if (prod[i].codigo == prod[*cont].codigo) { // Verifica se o código já existe
    ...
    do {
        ...
        strcpy(prod[*cont].nome, validar); // Copiar o nome do 'validar' para o 'prod[*cont].nome'
        for(int i = 0; i < *cont; i++){
            if(strcmp(prod[i].nome, prod[*cont].nome) == 0) { // Verifica se o nome já existe
    ...
    do {
        ...
        prod[*cont].preco = atof(validar); // Converte a string validar para float e adicionar o valor na variavel prod[*cont].preco  
    } while (!sair); 

    printf("\nPronto! Produto Cadastrado\n");
    (*cont)++; // +1 no contador de produtos cadastrados
}

void removerProduto (Produto prod[MAX_PROD], int *cont) {
    bool sair = false;
    int removerprod = 0, posicao = 0, tem = 0;
    
    if (*cont <= 0) { // Verifica se existem produtos cadastrados
        printf("\nNenhum produto foi cadastrado.\n");
        return; 
    }  
    
    visualizarListaProduto(prod, cont); // Exibe a função visualizarListaProduto, que vai perguntar se o usuário quer ver a lista de produtos
    ...
    for (int i = 0; i < *cont; i++) { // Loop para buscar o produto a ser removido
    ...
    if (tem == 1) {
        ...
        for (int i = posicao; i < *cont; i++) { // Move os produtos seguintes uma posição para trás, preenchendo o espaço do produto que foi removido.
            prod[i].codigo = prod[i+1].codigo;
            strcpy(prod[i].nome, prod[i+1].nome);
            prod[i].preco = prod[i+1].preco;
        }
        (*cont)--; // -1 no contador de produtos
...
void excluirTodosProdutos (Produto prod[MAX_PROD], int *cont) {
    bool sair;
    char escolha[3];
    ...
    do { // do-while caso o usuário tenha entrada por engano.
        ...
        case '1':
            ...
            (*cont) = 0; // Zera o contador de produtos.
...
void infoProduto (Produto prod[MAX_PROD], int *cont) {
    bool sair;
    char escolha[3];
    int codigotem = 0, tem = 0, posicao = 0;
...
// Funções sobre o Carrinho, acesso livre
void adicionarCarrinho (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    bool sair = false;
    int adicionarmais = 0, posicao = 0;

    if (*contcar > 49) { // Verifica se o limite de 50 produtos no carrinho foi atingido.
    ...
    do {
        ...
        for (int i = 0; i < *contcar; i++) {
            car[*contcar].produto = prod[posicao];
            if (car[i].produto.codigo == car[*contcar].produto.codigo) { // Verifica se o produto já está no carrinho.
                temnocar = 1;
                adicionarmais = 1;
                posicao = i;
                break;
    ...
    (*contcar)++; // +1 no contador do carrinho
}

void removerdoCarrinho (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    bool sair = false;
    int temnocar = 0, posicao = 0, removercar = 0;

    if (*contcar <= 0) { // Verifica se existem produtos cadastrados.
        printf("\nNenhum produto foi inserido no Carrinho.\n");
        return; 
    } 
    ...
    if (temnocar) {
        ...
        for (int i = posicao; i < *contcar; i++) { // Move os produtos seguintes uma posição para trás, preenchendo o espaço do produto que foi removido.
...
void fecharCarrinho (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    bool sair;
    int finalizar = 0;
    char escolha[3];
...
void cancelarCarrinho (int *contcar) {
    bool sair;
    char escolha[3];
    ...
    do {
        ...
        case '1':
            ...
            (*contcar) = 0; // Zera o contador de produtos.
            sair = true; // Define sair como verdadeiro para encerrar o loop.
            break;
...
void temNoCarrinho(Carrinho car[MAX_PROD], int *contcar) {
    bool sair = false;
    int codigotem = 0, tem = 0;
...
void areaAdmin (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    bool sair;
    char escolha[3];
    ...
    do {
        ...
        case '3':
            infoProduto(prod, cont);
            break;

        case '4':
            removerProduto(prod, cont);
            break;
        
        case '5':
            excluirTodosProdutos(prod, cont);
            break;
...
void areaCliente (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    bool sair;
    char escolha[3];
    ...
    do {
        ...
        case '3':
            adicionarCarrinho(prod, cont, car, contcar);
            break;
        ...
        case '5':
            temNoCarrinho(car, contcar);
            break;

        case '6':
            removerdoCarrinho(prod, cont, car, contcar);
            break;

        case '7':
            fecharCarrinho(prod, cont, car, contcar);
            break;

        case '8':
            cancelarCarrinho(contcar);
            break;
...
void verificarAcesso (Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    char user[11], senha[11];
    ...
    if (strcmp(user, "admin") == 0 && strcmp(senha, "admin123") == 0) {
        ...
        areaAdmin(prod, cont, car, contcar);
    } else {
...
void menu(Produto prod[MAX_PROD], int *cont, Carrinho car[MAX_PROD], int *contcar) {
    bool sair = false;
    char escolha[3];
    ...
    do {
        ...
        switch (escolha[0]) {
        case '1':
            areaCliente(prod, cont, car, contcar);
            break;

        case '2':
            verificarAcesso(prod, cont, car, contcar);
            break;
...
int main() {
    ...
    menu(prod, &cont, car, &contcar); // Chama a função menu, passando as informações dos produtos e do carrinho.
